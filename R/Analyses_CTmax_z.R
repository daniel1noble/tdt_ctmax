...
T <- c(44,46,48,50)
#################################################
# EXAMPLE CALCULATION WITH ENRICO
#################################################

mod_arstrong1 <- lm(log10(t1)~ T) # Need to have log10 time on y because it's the only thing that varies, so residual
summary(mod_arstrong1)
    z1 = 1/coef(mod_arstrong1)[2]
 se_z1 = sqrt(diag(vcov(mod_arstrong1))[2])^0.5 # convert se from original scale to new scale

ctmax1 = coef(mod_arstrong1)[1] / coef(mod_arstrong1)[2]
se_ctmax1 = sqrt(diag(vcov(mod_arstrong1))[1]*(1/coef(mod_arstrong1)[2])^2) # convert se from original scale to new scale
...
mod_cooper <- lm(log10(t) ~ T)
z_cooper <- 1 / coef(mod_cooper)[2]
ctmax_cooper <- coef(mod_cooper)[1] / coef(mod_cooper)[2]
