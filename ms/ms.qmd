...
check_ctmax <- predict_accuracy(data, ctmax, trait = ctmax)
 check_cfls <- predict_accuracy(data, cfts, trait = cfts) # Something wrong here.
  ...
  # Plot the data
    plot(check_z$tdt_z ~ check_z$est, ylab = "True Slope (z)", xlab = "Estimated Slope", pch = 16, cex = 2)
    abline(a = 0, b = 1, col = "red")
    plot(check_ctmax$ctmax ~ check_ctmax$est, ylab = "True CTmax", xlab = "Estimated CTmax", pch = 16, cex = 2)
    abline(a = 0, b = 1, col = "red")
    plot(check_cfls$cfts ~ check_cfls$est, ylab = "True CFLs", xlab = "Estimated CFLs", pch = 16, cex = 2)
    abline(a = 0, b = 1, col = "red")
...
Models including phylogeny were better supported than models without phylogeny ($\Delta DIC$ = `r mcmcglmm_model_z_trans_nophylo$DIC - mcmcglmm_model_z_trans$DIC`), with all traits exhibiting a moderate to strong phylogenetic signal (Phylogenetic $H^2$: $CT_{max}$ – `r posterior.mode(h2_ctmax)`, 95% CI: `r HPDinterval(h2_ctmax)[,"lower"]` to `r HPDinterval(h2_ctmax)[,"upper"]`; $z^{TDT}$ – `r posterior.mode(h2_z)`, 95% CI: `r HPDinterval(h2_z)[,"lower"]` to `r HPDinterval(h2_z)[,"upper"]`; CFLs – `r posterior.mode(h2_cfls)`, 95% CI: `r HPDinterval(h2_cfls)[,"lower"]` to `r HPDinterval(h2_cfls)[,"upper"]`). The model predicted known data reasonably well for most traits (correlation between actual and estimated data: $z^{TDT}$ – `r cor.test(check_z$tdt_z, check_z$est)$estimate`, 95% CI: `r cor.test(check_z$tdt_z, check_z$est)$conf.int[1]` to `r cor.test(check_z$tdt_z, check_z$est)$conf.int[2]`, p < 0.001).
...
# Plot 2
plot(check_z$tdt_z ~ check_z$est, ylab = "TDT Slope (z)", xlab = "Estimated TDT Slope", pch = 16, cex = 3, cex.lab = 2)
abline(a = 0, b = 1, col = "red")
