...
We can have a look at each new species' TDT curves collected by Vanessa (@fig-tdt). We can also use the data to calculate the TDT curve (slope) and $CT_{max}$ for each species (Table 1). 
...
Parameters from Thermal Death Time (TDT) curves provide a powerful tool for understanding the thermal sensitivity of organisms. But there are drawbacks to estimating TDT curves in comparison to other lethal (e.g. $CT_{max}$) and sublethal (CFL) measures. TDT curves are logistically challenging requiring more individuals for reliable estimates than other measures like $CT_{max}$ and CFL’s. TDT curves (and $CT_{max}$) also cannot be used for all organisms due to ethical and conservation considerations. Because of these limitations data on thermal sensitivity/heat vulnerability is missing for many species, creating significant knowledge gaps across the tree of life. 

Phylogenetic comparative approaches could be leveraged to help fill these knowledge gaps because closely related species are often more likely to share similar traits owing to their evolutionary history (phylogenetic niche conservatism) (ref). If heat tolerance is linked to the phylogeny, shared characteristics that make species more or less heat tolerant will not be randomly distributed across the phylogeny but biased by species groups (ref). We can can use evolutionary relatedness to predict species thermal sensitivities and such insights can be used to inform policy and conservation related decisions making for species with little to no data.

While the comparative approach can help fill knowledge gaps it does not entirely solve the issue that TDT curves cannot be estimated for many organisms (logistical and ethical). $CT_{max}$ is less logistically challenging but also has ethical limitations. Ethical issues could be alleviated by the use of sublethal measures of thermal tolerance like CFL’s (ref), but we do not know how well lethal (TDT curves/$CT_{max}$) and sublethal (CFL’s) measures capture thermal sensitivities/vulnerabilities. Identifying whether a correlation exists between lethal and sub-lethal limits will help us understand the mechanisms of vulnerability to heat stress that might lead to population extinction well before mortality is identified. And if these traits are correlated different measures of thermal tolerance can be used for different organisms.
